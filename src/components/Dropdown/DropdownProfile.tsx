...
const DropdownProfile: React.FC<{ user: IUser }> = ({ user }) => {
    ...
    const { userInfo, setOnlineUsers, setUserInfo, setSolBalance } = useUserProvider();
    ...
    const handleUpdateUser = async (data: boolean, type: string) => {
        const updateData = {
            id: userInfo!._id,
            [type]: data
        }
        const res = await fetchWithAuth(`/api/auth/online`, {
            method: 'POST',
    ...
    return (
        <div className="flex h-full items-center w-fit md:w-auto relative inline-block text-left" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute right-0 top-0 mt-[75px] w-[266px] z-[3000] bg-[#1D1D1D] border border-[#303030] rounded-lg focus:outline-none transition duration-200 ease-out">
